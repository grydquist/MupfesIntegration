...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!     This routine selects boundary element type for NURB boundary elements
...
!#####################################################################
      ...
      IF (ALLOCATED(this%bs)) THEN
         ...
         DEALLOCATE(this%bs)
      END IF
      IF (ALLOCATED(this%INN)) DEALLOCATE(this%INN)
      IF (ALLOCATED(this%nW))  DEALLOCATE(this%nW)


      SUBROUTINE comuMsh(lM, cm)
!     And distributing bs for NURBS
      ...
      SUBROUTINE partMsh(lM, cm, wgt)
!     And distributing bs for NURBS
      IF (lM%eType .EQ. eType_NRB) THEN
         a = lM%bs(2)%nEl
         IF (nsd .EQ. 3) a = a*lM%bs(3)%nEl
         DO i=0, cm%np()
            lM%eDist(i) = NINT(SUM(fwgt(1:i))*lM%bs(1)%nEl)*a
         END DO
      ELSE
...
!     Now scattering the sorted lM%INN to all processors
         CALL cm%scatter(tempIEN,lM%INN)
      END IF
